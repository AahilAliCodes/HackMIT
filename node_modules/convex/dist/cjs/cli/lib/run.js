...
async function runPaginatedQuery(ctx, deploymentUrl, adminKey, functionName, componentPath, args, limit) {
  const results = [];
  let cursor = null;
  let isDone = false;
...
async function runQuery(ctx, deploymentUrl, adminKey, functionName, componentPath, args) {
  let onResult;
...
async function subscribeAndLog(ctx, deploymentUrl, adminKey, functionName, args, componentPath) {
  return subscribe(
    ctx,
    deploymentUrl,
    adminKey,
...
async function subscribe(ctx, deploymentUrl, adminKey, functionName, args, componentPath, until, callbacks) {
