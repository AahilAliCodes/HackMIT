...
__export(bundle_exports, {
  ...
  encodeDefinitionPath: () => encodeDefinitionPath,
  getDeps: () => getDeps
});
...
function encodeDefinitionPath(s) {
  const components = s.split(import_path.default.sep);
  return components.map((s2) => {
    const escaped = s2.replace(/[^a-zA-Z0-9_]/g, "_");
    if (escaped.length <= 64 && escaped === s2) {
      return escaped;
    }
    const hash = import_crypto.default.createHash("md5").update(s2).digest("hex");
...
async function bundleDefinitions(ctx, absWorkingDir, dependencyGraph, rootComponentDirectory, componentDirectories, liveComponentSources, verbose = false) {
  ...
  const componentDefinitionSpecsWithoutImpls = componentBundles.map(({ directory, outputJs, outputJsMap }) => ({
    definitionPath: encodeDefinitionPath(
      (0, import_directoryStructure.toComponentDefinitionPath)(rootComponentDirectory, directory)
    ),
    origDefinitionPath: (0, import_directoryStructure.toComponentDefinitionPath)(
      rootComponentDirectory,
      directory
    ),
    definition: {
...
async function bundleImplementations(ctx, rootComponentDirectory, componentDirectories, nodeExternalPackages, verbose = false) {
  ...
  for (const directory of [rootComponentDirectory, ...componentDirectories]) {
    ...
    if (ctx.fs.exists(import_path.default.resolve(resolvedPath, "schema.ts"))) {
      schema = (await (0, import_bundler.bundleSchema)(ctx, resolvedPath))[0] || null;
